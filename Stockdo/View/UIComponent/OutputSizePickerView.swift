...
class OutputSizePickerView: UIViewController {
    ...
    private func loadData() {
        // compact, full
        outputSize.removeAll()
        outputSize.append("compact")
        outputSize.append("full")
    ...
    @objc func donePressed() {
        ...
        let value = outputSize[selectedValue]
        self.delegate?.saveToUserDefault(value: value, key: "output")
    ...
    func loadPickerSelection() {
        if let outputSize = selectedOutputSize {
            setDefaultValue(item: outputSize, inComponent: 0)
        }
    }
    
    func setDefaultValue(item: String, inComponent: Int){
     if let indexPosition = outputSize.firstIndex(of: item){
       pickerView.selectRow(indexPosition, inComponent: inComponent, animated: true)
     }
    }
}

extension OutputSizePickerView: UIPickerViewDataSource {
...
extension OutputSizePickerView: UIPickerViewDelegate {
